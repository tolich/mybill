...
Ext.app.Layer=function(){
    var items = [];
    if (App.settings.account) items.push({
		id: 'account'
    ...
    if (App.settings.personal) items.push({
		id: 'personal'
		,title: 'Персональная'
		,collapsible: true
		,autoScroll: true
    ...
    if (App.settings.tariff) items.push({
		id: 'tariff'
        ...
        ,hidden: App.settings.tariff==false
		,iconCls: 'tariff'
		,tools:[
			{
			    id:'refresh',
    ...
    if (App.settings.message) items.push({
		id: 'message'
		,title: 'Сообщения'
		,collapsible: true
		,autoScroll: true
