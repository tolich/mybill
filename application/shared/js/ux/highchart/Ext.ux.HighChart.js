 ...
 *
 * 17-04-2010
 * - Fixed the destroy function (didnt remove the component)
 * + onMove: updates the chart's tooltip positions
 * + onContainerResize: Create's a new chart with the new height/width
 *
 * 15-04-2010
 * + Changed Ext.ux.ChartPanel to Ext.ux.HighChart
 * + Changed Panel to BoxComponent (Just like the Ext.chart.Chart class).
 * 
 */
/**
...
Ext.ux.HighChart = Ext.extend(Ext.BoxComponent, {
    ...
    /**
     *
     * Resize the chart when the box is resized.
     * this makes the chart loose it's state.
     */
    resizable: true,
    ...
    addSeries: function(series, append){
        ...
        Ext.each(series, function(serie){
            if (!serie.serieCls){
                if (serie.type!=null || this.defaultSerie!=null){
                    cls = Ext.ux.HighChart.Series.get(serie.type||this.defaultSerie)
                } else {
     ...
     * @param {boolean} initial Is this the first time.
     */
    ...
    refreshRow: function(record){
        ...
        for (var i = 0; i < this.chart.series.length; i++){
            ...
            var point = this.series[i].getData(record);
            serie.data[index].update(point);
        }

        if (this.xField){
            this.chart.xAxis[0].setCategories(this.getxAxisData(), true);
        }
    },
    ...
    onAdd : function(ds, records, index){
        ...
        for (var i = 0; i < records.length; i++){
            ...
            for (i = 0; i < this.chart.series.length; i++){
                var serie = this.chart.series[i], s = this.series[i];
                var point = s.getData(record);
...
Ext.ux.HighChart.PieSerie = Ext.extend(Ext.ux.HighChart.Serie, {
    ...
    /**
     * Add data, for counting the totals.
     */
    ...
    /**
     * Returns the data like highchart wants.
     */
    getData: function(record){
        if (this.useTotals){
            this.addData(record);
