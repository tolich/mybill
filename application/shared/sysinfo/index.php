<?php
/* $Id: index.php,v 1.152 2008/02/01 05:30:49 jacky672 Exp $
 * 
...
if(isset($_GET['lang']) && $_GET['lang'] != "") {
  $file_name = basename($_GET['lang']);
  if(file_exists('language/'.$file_name.'.xml')) {
    $lang = $file_name.'.xml';
  }
}
...
<div id="container" style="display: none;">  
  ...
  <span lang='045'>L</span>
  <select name="lang" onChange="document.theme.submit();">
  <?php
  ...
  foreach($dirlist as $file) {
    ...
    if($lang_ext == ".xml") {
        if($lang_name.".xml" == $lang) {
          echo '<option name="' . $lang_name . '" selected="selected">' . $lang_name . '</option>';
        } else {
          echo '<option name="' . $lang_name . '">' . $lang_name . '</option>';
        }
    }
  }
  ?>
  ...
  <div id="ups" style="display: none;">
    <h2 lang='068'>UPS information</h2>
    <table class="stripeMe" id="upsTable" cellspacing="0">
    </table>    
  </div> 
