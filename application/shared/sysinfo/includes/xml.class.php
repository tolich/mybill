<?php
/* $Id: xml.class.php,v 1.17 2008/01/27 14:17:25 jacky672 Exp $
*
	...
	private function buildUpsinfo() {
		...
		if (isset($arrBuf) && !empty($arrBuf)) {
			...
			for ($i = 0, $max = sizeof($arrBuf);$i < $max;$i++) {
				...
				$item->addChild('Name', $arrBuf[$i]['name']);
				$item->addChild('Model', $arrBuf[$i]['model']);
				$item->addChild('Mode', $arrBuf[$i]['mode']);
				$item->addChild('StartTime', $arrBuf[$i]['start_time']);
				$item->addChild('Status', $arrBuf[$i]['status']);
				$item->addChild('UPSTemperature', $arrBuf[$i]['temperature']);
				$item->addChild('OutagesCount', $arrBuf[$i]['outages_count']);
				$item->addChild('LastOutage', $arrBuf[$i]['last_outage']);
				$item->addChild('LastOutageFinish', $arrBuf[$i]['last_outage_finish']);
