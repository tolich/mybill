/** $Id: phpsysinfo.js,v 1.28 2008/02/02 10:45:06 jacky672 Exp $
...
function changeLanguage(lang) { 
  ...
  //$("[@lang]").removeAttr("lang") //crash IE6
}
...
function populateFilesystems(xml) {
  ...
  $("FileSystem",xml).each(function(id) {
    ...
    $("Mount",filesystem).each(function(mid) {
      ...
      $("#filesystemTable").append("<tr><td val=\"" + mpoint + "\">" + mpoint + "</td><td val=\"" + type + "\">" + type + "</td><td val=\"" + name + "\">" + name + "</td><td val=\"" + percent + "\">" + createBar(percent) + inodes_text + "</td><td val=\"" + free + "\">" + formatBytes(free) + "</td><td val=\"" + used + "\">" + formatBytes(used) + "</td><td val=\"" + size + "\">" + formatBytes(size) + "</td></tr>");
  ...
  if(checkForUPSinfo(xml)) {    
    $("UPSinfo",xml).each(function(id) {
      ...
      $("Ups", upses).each(function(did) {
        ...
        $("#upsTable").append('<tr><td colspan="2" style="text-align: center"><strong>' + name + ' (' + mode + ')</strong></td></tr>');
        $("#upsTable").append('<tr><td lang="070">Model</td><td>' + model + '</td></tr>');
        $("#upsTable").append('<tr><td lang="072">Started</td><td>' + start_time + '</td></tr>');
        $("#upsTable").append('<tr><td lang="073">Status</td><td>' + status + '</td></tr>');

		if (temperature != '') $("#upsTable").append('<tr><td lang="084">Temperature</td><td>' + temperature + '</td></tr>');
        if (outages_count != '') $("#upsTable").append('<tr><td lang="074">Outages</td><td>' + outages_count + '</td></tr>');
        if (last_outage != '') $("#upsTable").append('<tr><td lang="075">Last outage cause</td><td>' + last_outage + '</td></tr>');
        if (last_outage_finish != '') $("#upsTable").append('<tr><td lang="076">Last outage timestamp</td><td>' + last_outage_finish + '</td></tr>');
        if (line_voltage != '') $("#upsTable").append('<tr><td lang="077">Line voltage</td><td>' + line_voltage + '&nbsp;<span lang="082">V</span></td></tr>');
        if (load_percent != '') $("#upsTable").append('<tr><td lang="078">Load percent</td><td>' + createBar(load_percent) + '</td></tr>');
        if (battery_voltage != '') $("#upsTable").append('<tr><td lang="079">Battery voltage</td><td>' + battery_voltage + '&nbsp;<span lang="082">V</span></td></tr>');
        $("#upsTable").append('<tr><td lang="080">Battery charge</td><td>' + createBar(battery_charge_percent) + '</td></tr>');
        $("#upsTable").append('<tr><td lang="081">Time left on batteries</td><td>' + time_left_minutes + '&nbsp;<span lang="083">minutes</span></td></tr>');
      });
    });
    $("#ups").show();
  } //if we have upses
...
function displayErrors(xml) {
  ...
  document.title = "phpSysInfo | Houston, we got a problem.";
